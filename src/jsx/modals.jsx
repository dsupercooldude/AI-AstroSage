...
window.AuthModal = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 gl-fadein"><div className="w-full max-w-sm rounded-3xl bg-[#09090b] p-6 shadow-2xl border border-[#27272a] relative">
      <form onSubmit={handleSubmit}><SageLogo size={44}/><h2 className="text-center font-serif text-2xl mt-1 mb-4 text-amber-200">{mode==="signup"?"Create Account":mode==="reset"?"Reset Password":"Sign In"}</h2>{err && <div className="text-[10px] text-red-300 bg-red-900/30 p-2.5 mb-3 rounded-xl border border-red-500/20">{err.startsWith("DECRYPTION_FAILED:") || err.startsWith("DEVICE_LOCKED:") ? (<div><p className="mb-2 text-red-200">Vault locked to original browser. The encryption keys are missing on this device.</p>  {!showImport && !confirmWipe && (    <>      <button type="button" onClick={() => setShowImport(true)} className="w-full py-2 bg-blue-500 hover:bg-blue-400 transition text-white font-bold rounded mb-2">Import Keys from Old Browser</button>      <button type="button" onClick={() => setConfirmWipe(true)} className="w-full py-1.5 bg-red-900/50 hover:bg-red-900/80 transition text-red-300 border border-red-500/30 text-[10px] rounded">Delete Cloud Data & Start Fresh</button>    </>  )}  {showImport && (    <div className="mt-2 space-y-2 text-left bg-black/60 p-3 rounded border border-blue-500/30">      <label className="text-[10px] text-blue-300 font-bold block">Paste Exported Key String:</label>      <textarea value={importKeyStr} onChange={ev=>setImportKeyStr(ev.target.value)} className="w-full h-16 bg-[#09090b] border border-[#27272a] rounded p-2 text-[10px] font-mono text-slate-300 outline-none resize-none"></textarea>      <div className="flex gap-2">        <button type="button" onClick={() => setShowImport(false)} className="flex-1 py-1.5 bg-slate-800 text-white text-xs rounded">Cancel</button>        <button type="button" onClick={() => {           if(importKeyStr.trim()) {             const success = CryptoUtils.importKeys(importKeyStr.trim());             if(success) { setErr(''); setShowImport(false); setImportKeyStr(""); } else { setImportKeyStr("Invalid Key String"); }           }        }} className="flex-1 py-1.5 bg-blue-500 text-white font-bold text-xs rounded">Import</button>      </div>    </div>  )}  {confirmWipe && (    <div className="mt-2 space-y-2 bg-red-950/60 p-3 rounded border border-red-500/50 text-left">      <p className="text-[10px] text-red-300 font-bold leading-tight mb-2">PERMANENTLY ERASE your cloud vault and reset it for this device? You cannot undo this.</p>      <div className="flex gap-2">        <button type="button" onClick={() => setConfirmWipe(false)} className="flex-1 py-1.5 bg-slate-800 text-white text-xs rounded">Cancel</button>        <button type="button" onClick={async () => {          setConfirmWipe(false);          const emailHash = await AppDB.hashKey(e.trim().toLowerCase());          const vault = await AppDB.getFile(`gl_vault_${emailHash}.json`);          vault.content = { profiles: [], settings: {} };          await AppDB.saveFile(`gl_vault_${emailHash}.json`, vault.content, vault.sha);          setErr('Vault wiped successfully. You can now sign in fresh.');        }} className="flex-1 py-1.5 bg-red-600 hover:bg-red-500 text-white font-bold text-xs rounded">ERASE VAULT</button>      </div>    </div>  )}</div>) : err}</div>}<div className="space-y-3"><div><label className="text-[10px] t40 uppercase font-mono">Email Address (optional for passkey)</label><input type="email" value={e} onChange={ev=>setE(ev.target.value)} className="w-full bg-black/40 border border-[#27272a] rounded-xl px-3 py-2.5 text-sm outline-none text-white focus:border-amber-400/50"/></div>{mode==="login" && <div><label className="text-[10px] t40 uppercase font-mono">Password</label><input type="password" value={p} onChange={ev=>setP(ev.target.value)} className="w-full bg-black/40 border border-[#27272a] rounded-xl px-3 py-2.5 text-sm outline-none text-white focus:border-amber-400/50"/></div>}</div><button type="submit" className="w-full bg-amber-400 text-black font-semibold rounded-full py-3 mt-5 hover:bg-amber-300 transition shadow-lg shadow-amber-400/20">{mode==="signup"?"Generate Credentials":mode==="reset"?"Reset Password":"Enter Vault"}</button>{mode==="login" && <button type="button" disabled={passkeyBusy} onClick={handlePasskeyLogin} className="w-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 font-semibold rounded-full py-3 mt-2 hover:bg-emerald-500/25 transition"><Icon name="fingerprint" size={16} /> {passkeyBusy ? "Verifying Passkey..." : "Use Face ID / Passkey"}</button>}
...
window.AdminConsoleModal = ({ onClose, onResetDb }) => {
  const { Icon, AppDB, CryptoUtils } = window;
  const [o, setO] = useState(AppDB.config?.owner || ""); const [r, setR] = useState(AppDB.config?.repo || ""); const [t, setT] = useState(AppDB.config?.token || ""); const [adminMfaSetup, setAdminMfaSetup] = useState(null); const [confirmDis, setConfirmDis] = useState(false);
  const handleSaveDb = async (e) => { e.preventDefault(); await AppDB.setConfig(o, r, t); onClose(); };
  ...
  const verifyAdmin2FA = async () => { const totp = new window.OTPAuth.TOTP({ secret: adminMfaSetup.secret }); if (totp.validate({ token: adminMfaSetup.pin, window: 1 }) === null) { setAdminMfaSetup({...adminMfaSetup, pin: ''}); return; } let adminFile = await AppDB.getFile('gl_admin.json'); adminFile.content.mfa = await CryptoUtils.encrypt(adminMfaSetup.secret); await AppDB.saveFile('gl_admin.json', adminFile.content, adminFile.sha); setAdminMfaSetup(null); };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 p-4" onClick={onClose}><div onClick={e=>e.stopPropagation()} className="w-full max-w-md bg-[#09090b] p-6 rounded-3xl border border-[#27272a] shadow-2xl gl-fadein max-h-[85vh] overflow-y-auto">
      ...
      <div className="mt-5 pt-4 border-t border-[#27272a] space-y-3"><span className="font-mono text-[9px] uppercase t50 block">Admin Security & 2FA</span>
        ...
        {!confirmDis ? <button type="button" onClick={()=>setConfirmDis(true)} className="w-full bg-red-500/20 border border-red-500/30 text-red-300 font-bold rounded-full py-2.5 hover:bg-red-500/40 transition">Disconnect Database</button> : ( <div className="p-3 bg-red-950/50 rounded-xl border border-red-500/30 text-center"><p className="text-[10px] text-red-300 mb-2 font-bold">Disconnect Database? You will be logged out.</p><div className="flex gap-2"><button type="button" onClick={()=>setConfirmDis(false)} className="flex-1 py-1.5 bg-slate-800 text-white rounded text-xs">Cancel</button><button type="button" onClick={onResetDb} className="flex-1 py-1.5 bg-red-600 text-white font-bold rounded text-xs">Disconnect</button></div></div> )}
      </div></div></div>
  );
};
