...
const hashTarget = `const hashedInput = await CryptoUtils.hashPassword(p);`;
const hashReplace = `const hashedInput = await CryptoUtils.hashPassword(p.trim());`;
c = c.replace(hashTarget, hashReplace);

const matchTarget = `if(u.p !== p && u.p !== hashedInput)`;
const matchReplace = `if(u.p !== p.trim() && u.p !== hashedInput)`;
c = c.replace(matchTarget, matchReplace);
